...
//						playingPattern = constrain(playingPattern + amt, 0, 7);
						...
						// SET PATTERN LENGTH
						auto newPatternLen = constrain(PatternLength(seqState.playingPattern) + amt, 1, NUM_STEPS);
						SetPatternLength( seqState.playingPattern, newPatternLen );
					...
					// NOTE SELECT MODE
						...
						if (nsparam > 10 && nsparam < 14){
							if(u.dir() < 0){			// RESET PLOCK IF TURN CCW
								...
								stepNoteP[seqState.playingPattern][selectedStep].params[tempmode] = -1;
							}
						}
						// PAGE ONE
...
//								Serial.println(thisKey-3);
						...
						// SEQUENCE 1-16 STEP KEYS
							...
							if (keyState[1] && keyState[2]) {		// F1+F2 HOLD
								if (!stepRecord && !patternParams){ // IGNORE LONG PRESSES IN STEP RECORD and Pattern Params
									if (keyPos <= getPatternPage(patternSettings[seqState.playingPattern].len) ){
	...
	//							}
								if ( stepNoteP[seqState.playingPattern][seqKey].trig == TRIGTYPE_PLAY || stepNoteP[seqState.playingPattern][seqKey].trig == TRIGTYPE_MUTE ) {
									stepNoteP[seqState.playingPattern][seqKey].trig = ( stepNoteP[seqState.playingPattern][seqKey].trig == TRIGTYPE_PLAY ) ? TRIGTYPE_MUTE : TRIGTYPE_PLAY;
								}
							}
						}
					}
